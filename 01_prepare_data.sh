...
for_each subjects/* : dwi2response dhollander IN/dwi.mif IN/response_wm.txt IN/response_gm.txt IN/response_csf.txt -force

# Get mean for each of the typees of tissues
responsemean subjects/*/response_wm.txt group_average_response_wm.txt -force
responsemean subjects/*/response_gm.txt group_average_response_gm.txt -force
responsemean subjects/*/response_csf.txt group_average_response_csf.txt -force

# Upsampling DW images 
for_each subjects/* : mrgrid IN/dwi.mif regrid -vox 1.25 IN/dwi_upsampled.mif -force
